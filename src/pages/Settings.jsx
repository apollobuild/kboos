...
const API_LABELS = {
  ...
  wati_url: 'WATI Server URL — must include your tenant ID, e.g. https://live-mt-server.wati.io/<tenant-id> (copy from WATI → API Docs)',
  apollo: 'Apollo.io',
  ...
  },
  {
    ...
    providers: [
      ...
      { key: 'wati', label: 'WATI (WhatsApp)', icon: '💬', color: '#00d97e', desc: 'WhatsApp Business messaging',
        ...
        extraKey: 'wati_url', extraLabel: 'WATI Server URL — must include your tenant ID, e.g. https://live-mt-server.wati.io/<tenant-id> (copy from WATI → API Docs)',
      },
                ...
                <div key={section.label} style={{ marginBottom: navCollapsed ? 8 : 20 }}>
                  ...
                  {section.items.map(item => (
                    ...
                    >
                      ...
                      {!navCollapsed && <span style={{ textTransform:'uppercase', letterSpacing:'0.07em', fontWeight: tab === item.id ? 600 : 400 }}>{item.label}</span>}
                    </div>
                  ))}
                </div>
              );
